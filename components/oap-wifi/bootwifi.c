 ...
 *
 * Check stored wifi settings on startup and turn into Access Point if there's no SSID defined.
 * The same effect can be achieved by pressing down control button during startup.
 *
 * In AP mode, sensor creates ddns-client-XXXX network (default password: duckduckgo),
		...
		case SYSTEM_EVENT_STA_GOT_IP: {
			...
			switch (mode) {
				...
				case WIFI_MODE_AP:
					...
					//ESP_LOGI(tag, "****** IP (AP %s) http://"IPSTR, ap_config.ap.ssid, IP2STR(&event->event_info.got_ip.ip_info.ip));
					//wifi_state_change(true, true);
					break;
				default:
...
static void become_access_point() {
	...
	sprintf((char*)ap_config.ap.ssid, "ddns-client-%02X%02X%02X%02X", mac[0], mac[1], mac[4], mac[5]);
